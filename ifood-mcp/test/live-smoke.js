...
const { makeIfoodBridge } = require('../src/ifood-bridge');
const { makeSupabase } = require('../src/supabase');
const { makeAuditor } = require('../src/audit');
const { readTools } = require('../src/registry');

const LABEL = 'cd-ifood-mcp';
// Chamadas de prova (args mínimos — o Bridge resolve o merchant da CD pelo default).
const CALLS = [{ tool: 'ifood_status', args: {} }];
...
async function main() {
  ...
  for (const { tool, args } of CALLS) {
    ...
    try {
      ...
      // Erro de negócio do Bridge (ex.: merchant não configurado) ainda prova o wiring + audit.
