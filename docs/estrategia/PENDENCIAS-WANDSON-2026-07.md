...
| #11 — branch `wandson/cora-situacao-dashboard` | ✅ **RESOLVIDO — a branch não existe mais** | `git ls-remote --heads origin` em 29/07: 0 matches para `situacao`/`dashboard`. Foi deletada em algum momento entre 05/07 e hoje. A recomendação de 05/07 ("não mergear, só deletar") foi cumprida |
