...
> **5ª atualização em 2026-07-06** — anexada a **Rodada 3** do smoke live: fluxo de ESCRITA real Merchant (pausar/despausar loja, POST 201 + DELETE 204 via draft→aprovação) confirmado contra o sandbox. M4 sobe de leitura-confirmada pra ✅ CONFIRMADO LIVE completo.
...
| M4 | `GET/POST /merchants/{id}/interruptions` + `DELETE .../interruptions/{id}` (pausar/despausar loja) | ✅ IMPLEMENTADO + ✅ CONFIRMADO LIVE — leitura, POST (criar pausa, 201) e DELETE (remover pausa, 204) todos exercidos ao vivo no fluxo completo draft→aprovação | `listarInterrupcoes` (`lib/ifood.js:436`), `criarInterrupcao` (`:444`), `removerInterrupcao` (`:459`) — distintos de `pausarItem`/`reabrirItem` (`:398,409`, que são **Catalog**/item de cardápio, não a loja inteira) | `GET /ifood-api/merchant-interruptions/:lojaId` (`routes/ifood-api.js:97`); escrita via `OPERACOES_ESCRITA['ifood.pausar_loja'\|'ifood.despausar_loja']` (`routes/ifood.js:33-34`), draft→aprovação como as demais | `TabMerchantIfood` (`LojaWorkspace.jsx:522`) — form "Pausar loja" (início/fim/motivo) + lista de pausas ativas com botão "Remover", ambos criando draft amarelo (nunca chamam a API direto); fluxo smoke live testou a API diretamente, não esta tela | Offline: `#762` adicionou testes próprios em `test/ifood.test.js` e `test/ifood-routes-acao-aprovar.test.js`. **Smoke live itens 5 e R3.1-R3.7** (Rodadas 1 e 3): leitura confirmada (item 5, `interrupcoes:[]`); fluxo de ESCRITA completo confirmado ao vivo — `POST /ifood/acao`→`/ifood/aprovar` criou a pausa de verdade no iFood (R3.3, `resultado.id` = prova do `201` upstream), `GET` confirma a pausa depois que `start` passa a valer (R3.4b — achado: pausas futuras não aparecem na listagem até a hora chegar, comportamento do iFood), e o mesmo fluxo removeu a pausa (R3.6, `resultado:null` = prova do `204` upstream) com sandbox devolvido ao estado limpo (R3.7) |
...
**Rodada 1** (commit `19752be` — inclui #758/#762/#763), **Rodada 2** (commit `3b06008` — inclui #768/#771, confirma redeploy automático via PM2 entre as duas rodadas) e **Rodada 3** (mesmo commit `3b06008` — única rodada com ESCRITA real, autorizada e restrita ao merchant sandbox). 10 itens read-only + 1 fluxo completo de escrita (pausar→confirmar→despausar), **todos verdes** — nenhum erro inesperado, nenhum efeito colateral (tokens/segredos/JWT/senhas nunca saíram da shell SSH; arquivos temporários removidos; sandbox devolvido ao estado limpo).
...
### Rodada 3 — usuários QA + fluxo de ESCRITA no sandbox (pausar/despausar loja)

**Escopo**: única rodada com ESCRITA real — autorizada explicitamente, restrita ao merchant SANDBOX `92a0ec17-...5f1`, revertida ao final da própria sessão. Usuário `qa-homolog@consultdelivery.com.br` criado (role `admin` em `cd-homolog`, RBAC `admin` confirmado) via runbook `docs/runbooks/homolog-demo-users.md`; senha só em arquivo local fora de git, nunca neste relatório. Um 2º usuário (`qa-demo`) foi criado mas não exercitado nesta rodada.

| # | Passo | HTTP | Resumo do body |
|---|---|---|---|
| R3.1 | Login `qa-homolog` via Supabase Auth REST | 200 | JWT obtido (nunca impresso, só usado em memória na sessão SSH) |
| R3.2 | `POST /ifood/acao` `{operacao:"ifood.pausar_loja", parametros:{start,end,description}}` | 200 | `{draft_id, operacao:"ifood.pausar_loja", content:"Pausar loja no iFood (...)"}`; `agent_drafts` confirmado no banco: `status:"pending"`, `autonomy_level:"amarelo"` |
| R3.3 | `POST /ifood/aprovar/:draftId` — COMMIT real no iFood | 200 (bridge) | `resultado:{id:"ab55b16c-...", start, end, description}` — **o `id` retornado é a prova do `201` upstream** (o bridge sempre envelopa sucesso em 200; o iFood criou a interrupção de verdade) |
| R3.4 | `GET /ifood-api/merchant-interruptions/:lojaId` (imediatamente após R3.3) | 200 | `interrupcoes: []` — vazio, apesar da pausa já criada (ver achado abaixo) |
| R3.4b | Mesma rota, retry ~40s depois | 200 | `interrupcoes:[{id:"ab55b16c-...", start, end, description}]` — a pausa aparece assim que o horário `start` passa a valer |
| R3.5 | `POST /ifood/acao` `{operacao:"ifood.despausar_loja", parametros:{interruption_id}}` | 200 | `{draft_id, operacao:"ifood.despausar_loja", content:"Remover pausa da loja no iFood (...)"}` |
| R3.6 | `POST /ifood/aprovar/:draft2Id` — DELETE real no iFood | 200 (bridge) | `resultado: null` — **bate com `204` sem corpo** (`tolerant()` devolve `null` em corpo vazio) |
| R3.7 | `GET /ifood-api/merchant-interruptions/:lojaId` (confirmação final) | 200 | `interrupcoes: []` — pausa removida com sucesso, sandbox devolvido ao estado limpo |

**Achado real (não é bug — comportamento documentado da API Merchant do iFood)**: `GET merchant-interruptions` não lista pausas agendadas para o futuro — só aparecem depois que o horário `start` chega (confirmado pelo intervalo R3.4→R3.4b). Implicação prática: qualquer UI/QA que crie uma pausa "a partir de agora" e confira a lista na hora verá vazio por até ~1min — comportamento do iFood, não do bridge.

### O que o smoke NÃO cobriu
...
- **PUT de horários (opening-hours)** — a Rodada 3 exercitou escrita só de interruptions (pausar/despausar loja); `atualizarHorarios` segue sem smoke (M5 escrita).
- **Front/UI** — a Rodada 3 testou o fluxo via chamada direta à API do bridge (`POST /ifood/acao` → `/ifood/aprovar`), não pela tela `TabMerchantIfood`; o smoke visual via browser/Console ficou fora do escopo desta rodada.
...
- **Smoke live 2026-07-06 rodado (ver seção dedicada acima)** — 10 itens read-only + 1 fluxo completo de ESCRITA, todos verdes, contra o Bridge real na VPS. Confirma ao vivo: filtro de data (R2), erros 400/404 (M6), 0-reviews→null do summary (R8), leitura de status/interruptions/opening-hours (M3/M4/M5), descobre o shape real de `merchant-status` (array, não objeto) — **e a Rodada 3 confirma o fluxo de ESCRITA Merchant ponta-a-ponta**: pausar loja (POST/201) e despausar (DELETE/204) via draft→aprovação, com sandbox devolvido ao estado limpo. **O que ainda segue smoke-pendente**: shape completo de 1 review (R1b), aritmética do summary com reviews de verdade (R8), 200 de detalhe de review com dado real (R4), e o PUT de horários (M5 — só interruptions foi exercitado na escrita).
...
5. ~~**Smoke live obrigatório antes da sessão**~~ — **RODADO em 2026-07-06, 10 itens read-only + fluxo de escrita Merchant completo, todos verdes (ver seção "Smoke live" acima).** Resta uma fatia menor e mais difícil de fechar sem dado real: pelo menos 1 review de verdade no sandbox pra confirmar o shape completo do objeto review (R1b), a aritmética do summary (R8) e o 200 de detalhe de review (R4) — hoje o sandbox está com 0 reviews. PUT de horários (M5) também segue sem smoke — só a escrita de interruptions (M4) foi exercitada na Rodada 3.
