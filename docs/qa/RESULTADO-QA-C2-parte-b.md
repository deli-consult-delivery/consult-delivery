...
| `chat` | `console/chat/engine/useConversas.js` LIMIT_CONVS=150 | 150 | **177** (Consult) | ✅ CORRIGIDO (#872, 2026-07-07) — badges da barra de filtros (aguardando/abertos/automação/resolvidos/falha/arquivados) agora usam `count: 'exact', head: true` por `status_v2` (7 queries paralelas). `interno` (canais internos) continua derivado de `extraConvs` (já contado corretamente). Fallback do array se counts ainda não carregaram/erro (sem flash de "0") |
...
| `radar` | `RadarReal.jsx:238,255` | 500 | não medido (mesmo padrão) | ✅ CORRIGIDO (#873, 2026-07-07) — KPI "R$ defendido (Defesa)" total de casos e "X por atraso" agora usam `count: 'exact', head: true` (2 queries paralelas, filtros `porLoja`/`tenant_id` respeitados). Dívida honesta: `defendidoCentavos` (soma) segue do array capado (rótulo "aprox. (500 recentes)") — aggregação exigiria RPC/SUM server-side |
