...
- max_knowledge_base.max_kb_write: ✅ CONVERTIDA 2026-07-02 (migration `20260702_012`). Ramo global (tenant_id NULL, 7 linhas) preservado idêntico (qualquer admin/owner/deli_owner); ramo por-tenant agora correlacionado hierárquico via `accessible_tenant_ids_with_role(['admin','owner','deli_owner'])` — fecha o buraco de escrita cross-tenant latente. Teste: agência desce a todos stores; store-only isola; role `consultor` negada. **0 policies flat remanescentes → Rota B fechada.**
