...
**🔴 RISCO REAL ACEITO: ACESSO TOTAL AO GMAIL PRINCIPAL, NÃO READONLY.** O fluxo padrão de conexão do Gmail concedeu escopo `https://mail.google.com/` — leitura, envio, modificação e exclusão de mensagens na caixa `wandson@consultdelivery.com.br` — mais escopos de Google Contacts e dados pessoais (endereço, aniversário, telefone). **Não é `gmail.readonly`.** Confirmado via `tokeninfo` do Google (fonte de verdade, não a config do Composio):
```
"scope": "email profile https://mail.google.com/ https://www.googleapis.com/auth/contacts.other.readonly
 https://www.googleapis.com/auth/contacts.readonly https://www.googleapis.com/auth/profile.emails.read ..."
```
Tentativa de forçar escopo customizado `gmail.readonly` via CLI (`composio dev auth-configs create`) falhou silenciosamente — provável limitação do plano/workspace consumer. Tentativa de criar um MCP server customizado restrito só a tools de leitura (via REST API) também bloqueada — a API key interna do CLI (`uak_...`) não é aceita pela API pública (`Invalid API key`, testado com headers `x-api-key` e `Authorization: Bearer`); criar uma via dashboard exigiria ação manual adicional do Wandson, não feita.

Parei e reportei ao Wandson com a evidência bruta duas vezes (uma vez comigo diretamente, outra via confirmação com a orquestradora da sessão) antes de prosseguir. Ele confirmou explicitamente, ciente do escopo total: **"Sim, foi eu, aceito escopo total."**

**Mitigação real em vigor (única camada técnica de restrição):** o recurso "Enhanced Control" (BETA) do Composio bloqueia as categorias Write (15 tools) e Destructive (7 tools) **na camada da plataforma Composio**, para esta conexão específica (`ca_XHEqVqSqFG8G`) — só Read (28 tools) fica habilitado. Isso é enforcement no backend do Composio, não no MCP client nem no token OAuth — o token em si continua com poder total; um bug ou mudança futura no Enhanced Control removeria essa proteção sem que o escopo OAuth mude.

**MCP no Nimbalyst:** registrado como `composio-ana-gmail`, transporte HTTP remoto (Streamable HTTP), URL `https://connect.composio.dev/mcp` (endpoint universal "Composio Connect" — expõe meta-tools que descobrem/executam qualquer tool do Gmail dinamicamente, não uma lista fixa restrita a leitura). Autorização confirmada (`outcome: 'authorized'` no log do Nimbalyst). **Decisão consciente de manter esse endpoint genérico** (em vez de um MCP customizado com allowlist de tools) dado que a API key para criar o server restrito não estava disponível — a única barreira de tool-scope é o Enhanced Control do Composio, descrito acima.

**Ainda não conectado à Ana:** o `config.yaml` do hermes-gateway (`/home/claudedev/.hermes/config.yaml`, onde a Ana busca seus MCP servers) não tem nenhuma entrada `gmail`/`composio` — confirmado (`grep -ic 'gmail\|composio'` = 0). Este MCP está registrado só no Nimbalyst (uso do Claude Code), não na Ana em produção. Conectar a Ana a esse Gmail é decisão/trabalho futuro, fora do escopo do CON-7 como executado.
