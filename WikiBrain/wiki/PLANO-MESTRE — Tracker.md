...
2g. **T4 (Hermes) 3B — GATE 0 concluído (Wandson) → SQL do `ceo_agent` AUTORADO (Opção A), aguardando `ok`.** `admin-mcp-design.md` completo; escopo `ceo_agent` = todos os tenants (✅ Wandson 2026-06-09); achado cross-tenant §2.1. **GATE 0 destravado → autorei o SQL** (`supabase/migrations/20260609_001_ceo_agent_hermes.sql`, Opção A: aditivo — coluna `agent_drafts.origin` com CHECK + índice parcial; enforcement no MCP, **sem** papel RBAC global; verificado contra a DB: tabela existe, `origin` ainda não, `status='pending'` é valor real). Versionado em git, **NÃO aplicado**. → **falta o Wandson:** (1) `ok` no SQL p/ eu aplicar (1 arquivo, output bruto, teste de isolamento RLS) · (2) criar usuário `claudedev` na VPS (§2 RUNBOOK). Aí sigo autônomo (tools de leitura + `cd_propor_draft` + gateway).
...
| T4 | Hermes | 🔄 | design admin-MCP ✅ + runbook ✅ + escopo `ceo_agent`=todos os tenants ✅ + **SQL do `ceo_agent` AUTORADO (Opção A, `20260609_001_ceo_agent_hermes.sql`, aditivo, verificado vs DB) ✅** — GATE 0 concluído pelo Wandson. **Aguarda: `ok` no SQL + `claudedev`** (Wandson); depois eu aplico + ligo as tools |
