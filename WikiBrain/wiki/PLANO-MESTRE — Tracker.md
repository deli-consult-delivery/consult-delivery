...
2g. **T4 (Hermes) 3B — migration APLICADA + admin MCP CONSTRUÍDO (sessão 30 cont. 8).** Wandson deu `ok` no SQL e mandou destravar tudo. **(1) Migration `20260609_001_ceo_agent_hermes.sql` APLICADA** (output bruto): coluna `agent_drafts.origin` + CHECK + índice parcial; **teste de isolamento RLS** (membro vê 3 / intruso vê 0). **(2) Módulo `admin-mcp/` construído e testado offline** — MCP stdio, 6 tools de leitura cross-tenant + `cd_propor_draft` (única escrita, sempre `pending`/`origin='hermes'`); enforcement estrutural (sem tool de aprovar/enviar/executar); config fail-closed (Infisical, nada no git); auditoria em toda chamada. Validação bruta: `node --check` (16 arquivos) · `npm run smoke` (5/5) · `npm run test:integration` (**handshake MCP real → 7 tools**). README documenta a subida. → **falta só o Wandson (credenciais/VPS):** criar `claudedev` · token `service_role` dedicado no Infisical · registrar o MCP no gateway do Hermes · smoke com credencial real. Aí a leitura é liberada.
...
5. **GATE 0 — ✅ CONCLUÍDO (2026-06-10).** Verificado ao vivo (Telegram · GitHub sem PAT de risco · EvoNexus aposentado · `claude-debug` inerte · plaintext limpo). Doc SSH stale corrigido (PR #291). → **próximo passo do Wandson: `claudedev`** (`docs/infra/claude-code-vps-setup.md`, RUNBOOK §2) — agente persistente não-root na VPS. Feito o claudedev, eu autoro o SQL do `ceo_agent` → `ok` → ligo as tools do Hermes.
...
| T4 | Hermes | 🔄 | **migration `ceo_agent` APLICADA** (`20260609_001`, `agent_drafts.origin` + índice; RLS isolamento membro=3/intruso=0) **+ admin MCP CONSTRUÍDO e com subida 100% documentada** (`admin-mcp/`: 6 tools leitura cross-tenant + `cd_propor_draft`; enforcement estrutural; auditoria total; smoke+integration+**live-smoke** verdes; comando real `hermes mcp add` no RUNBOOK — PR #289 `065bc09`). **Código/docs = 100%. Aguarda só o Wandson (credenciais/VPS): rodar `hermes mcp add cd-admin --env ...` + `npm run live-smoke`** → leitura liberada |
