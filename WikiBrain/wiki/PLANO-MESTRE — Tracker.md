...
2g. **T4 (Hermes) 3B — design fechado, escopo resolvido, aguarda GATE 0.** `admin-mcp-design.md` completo; **decisão de escopo do `ceo_agent` = todos os tenants** (✅ Wandson 2026-06-09); achado cross-tenant documentado (§2.1, Opção A recomendada). **Bloqueio real = GATE 0 + `claudedev`** (runbook `docs/infra/RUNBOOK-WANDSON.md`, reservado ao Wandson). SQL do `ceo_agent` **autorado na sessão de build pós-GATE 0**, não antes. → nada autônomo a fazer aqui até o GATE 0.
...
| T4 | Hermes | 🔄 | design admin-MCP ✅ + runbook do Wandson ✅ (`docs/infra/RUNBOOK-WANDSON.md`) + **escopo `ceo_agent` resolvido: todos os tenants** ✅ + achado cross-tenant documentado (§2.1, Opção A). **Aguarda só GATE 0 + `claudedev`** (Wandson); SQL autorado pós-GATE 0 |
