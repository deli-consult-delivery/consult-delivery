...
_Última sessão: 2026-06-09 (VPS — **sessão 30: orquestrador + LEVA 3 (Integrações) + reconciliação de docs stale**. Migration `20260609_004` (`tenant_integracoes`) + frontend prontos, build verde, **PR #275 aberto — aguarda `ok` do Wandson p/ aplicar**. Corrigido: T7 Onda 03 estava marcada "não aplicada" no Tracker — VERIFICADO em prod (9 conv/16 msg), já está no ar desde 2026-05-22; só falta Onda 04.)_
...
- **LEVA 3 (Integrações) — gated.** 2 executores em paralelo (`cd-migration-creator` + `cd-frontend-component`, arquivos disjuntos): migration `20260609_004_leva3_integracoes.sql` (`tenant_integracoes`, `tenant_id NOT NULL` + RLS **SELECT-only** via `is_member_of`, aditiva/reversível, espelha LEVA 2) + frontend `Integracoes({tenantDbId})` com `useRefRows`+`esc()` (XSS), badge via `ST_INT`, sem `CrudTela`. Build prod **verde** (exit 0). **PR #275 aberto.** ⏳ **Migration NÃO aplicada — aguarda `ok` do Wandson no SQL.**
...
- **Pós-`ok`:** aplicar `20260609_004` (1 arquivo · output bruto · parar no 1º erro) + RLS E2E (membro vê ≥0 / não-membro vê 0) + seed via service_role; depois mergear #275.
...
2f. **LEVA 3 — 🔄 EM ANDAMENTO (sessão 30, gated).** Tira a última tela de referência do mock: **Integrações** (`tenant_integracoes`). Migration `20260609_004` (tenant_id + RLS SELECT-only via `is_member_of`, aditiva/reversível, padrão LEVA 2) + frontend (`Integracoes({tenantDbId})` com `useRefRows`+`esc()`). Build verde. **PR #275 aberto.** ⏳ **Migration NÃO aplicada — aguarda `ok` do Wandson no SQL.** → próximo foco: aplicar pós-`ok` + RLS E2E + seed via service_role; depois mergear.
