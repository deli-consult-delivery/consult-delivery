...
| T4 | Hermes | 🔄 | **migration `ceo_agent` APLICADA** (`20260609_001`, `agent_drafts.origin` + índice; RLS isolamento membro=3/intruso=0) **+ admin MCP do Hermes CONSTRUÍDO** (`admin-mcp/`: 6 tools leitura cross-tenant + `cd_propor_draft`; enforcement estrutural; auditoria total; smoke+integration MCP verdes). **Aguarda só o Wandson (credenciais/VPS): `claudedev` + token `service_role` + registrar no gateway** → smoke com credencial real → leitura liberada |
