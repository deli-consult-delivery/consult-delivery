// server.js — bootstrap do evolution-mcp do Hermes (stdio). Molde ifood-mcp. SÓ LEITURA.
'use strict';
...
const { makeEvolutionBridge } = require('./evolution-bridge');
const { makeSupabase } = require('./supabase');
const { makeAuditor } = require('./audit');
const { allTools } = require('./registry');

function buildServer({ cfg, evolution, auditor }) {
  const server = new McpServer({ name: 'cd-evolution-mcp', version: '0.1.0' });
...
async function main() {
  ...
  const server = buildServer({ cfg, evolution, auditor });

  const transport = new StdioServerTransport();
  await server.connect(transport);
  process.stderr.write(`[cd-evolution-mcp] online (stdio) — ${allTools.length} tools (leitura), bridge=${cfg.bridgeUrl}\n`);
}

if (require.main === module) {
  main().catch((e) => {
    process.stderr.write(`[cd-evolution-mcp] FALHA no boot: ${e.message}\n`);
    process.exit(1);
  });
}
