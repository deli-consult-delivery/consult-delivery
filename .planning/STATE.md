...
**Last updated:** 2026-07-29 — verificação de estado da plataforma: base verde (build ✓, `tsc` exit 0, vitest 50/50 **no escopo do `include` — ~10 suítes de `trigger/`/`admin-mcp`/`vendaerp-mcp` ficam fora e não rodaram**, bridge 21/21, Bridge VPS `/health` ok, CI verde), 0 PRs abertos, nada quebrado no que foi executado. **Achado herdado que segue vivo em `main`: M2 do iFood App 3 (`unsellableItems` volta `{"categories":[...]}`, front espera array → "Nenhum item arquivado" sempre) — bloqueante de ticket, é o próximo item de código.** O `PLANO-MESTRE — Tracker.md` estava 22 dias atrasado (parou em #873, `main` em #889) e foi reconciliado com as 16 entregas de #874-#889. Anterior: 2026-07-26 — homologação Sandbox Cardápio Web → Venda ERP concluída; integração desligada em estado seguro.
**Last action:** OAuth da loja `11973` ativo com `orders store`; pedido Cardápio Web `53385` correlacionado a `CW-11973-53385` e Venda `3`, item `CW-HML-001`, estoque final `8` e único lançamento `626`. Status `confirmed` chegou como `Em preparação`. O PR #886 preservou `Finalizado/Lancado`, adicionou fence PUT/DELETE e impediu retry de 429; merge `f598189`, migrations aplicadas e Bridge saudável na VPS.
**Resume from:** decidir a data operacional para ativar novos pedidos; até lá, instalação `enabled=false`, chave geral de escrita `false` e produto Sandbox pausado.

---

## Sessões avulsas (fora do roadmap GSD)

- **2026-07-26 (Cardápio Web → Venda ERP, E2E Sandbox):** OAuth e webhook aprovados para a loja `11973`. Pedido `53385` entrou faturado no Venda `3`; o status `confirmed` foi refletido como `Em preparação`. Repetições controladas do PUT faturado mantiveram estoque `8` e somente o lançamento `626`. PR #886 mesclado e implantado com fence compartilhado entre PUT/DELETE e 429 sem retry; migrations aplicadas. Estado final seguro: instalação e escrita desativadas, produto de teste pausado.
