...
0. **Épico Dashboard iFood — FASE 4** (T6): filtro temporal universal — UI de janela de período (dia/semana/mês/custom) no `RadarReal.jsx` + ativar o ramo `periodo` do helper `lerMetricas` (`src/lib/radar-metricas.js`, dormente desde a Fase 1). Reusa botões-janela de `Execucoes.jsx` + date-range de `HistoricoCampanhas.jsx`; granularidade que a fonte não suporta → snapshot do período COM aviso honesto, nunca gráfico fabricado. Plano: `/root/.claude/plans/dynamic-swimming-zebra.md`. **FASE 1 ✅** (helper único de dedup, comportamento-neutro) · **FASE 2 ✅ EM PROD E VALIDADA** (sessão 49, #349, deploy `20260614.33` — parser enriquecido: 7 famílias de métricas aditivas via EAV; fix `data_ref` de Operação no `parsePeriodo`) · **FASE 3 ✅ ENTREGUE (sessão 50, só frontend — sem migration/sem deploy):** grid "Saúde financeira & cruzamentos" no `RadarReal.jsx` com 7 KPIs derivados das chaves REAIS do EAV (receita líquida, carga total iFood, taxa de cancelamento %, R$ perdido em cancelam., visitas não convertidas, avaliação vs piso Super 4,5, negociações), cada um guardado por `!= null` (some, não zera) + sinais enriquecidos (cancelamentos: motivo top/contestáveis/R$ em jogo; chamados). esbuild EXIT 0.
