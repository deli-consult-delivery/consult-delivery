...
0. **Épico Dashboard iFood — FASE 3** (T6): cruzamentos no `RadarReal.jsx` (Balde 2, só frontend — sem migration, sem deploy) — receita líquida, carga total iFood, taxa de cancelamento %, R$ perdido em cancelamentos, gap até o piso Super 4,5; cada KPI guardado por `!= null` (some, não zera, quando falta a fonte). Plano: `/root/.claude/plans/dynamic-swimming-zebra.md`. **FASE 1 ✅** (helper único de dedup, comportamento-neutro) · **FASE 2 ✅ EM PROD E VALIDADA** (sessão 49, #349, deploy `20260614.33` — parser `processar-fontes.ts` enriquecido: 7 famílias de métricas aditivas via EAV; o fix `data_ref` de Operação da sessão 48 estava quebrado e foi corrigido no `parsePeriodo` (formato curto `"Diário (09/05 a 07/06)"` + inferência de ano); gate validado por output bruto: `operacao.data_ref=2026-06-07`, métricas íntegras, nenhuma antiga sumiu).
