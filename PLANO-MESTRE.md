...
0. **Épico Dashboard iFood — FASE 2** (T6): enriquecer o parser `processar-fontes.ts` (Balde 1, aditivo via EAV) — repasse líquido de conciliação, ranking completo de cancelamentos, horário de pico/formas de pagamento (Vendas), itens cauda-longa (Cardápio), indicadores órfãos + Meta real (Operação), turnos (Logística). Sem migration; deploy Trigger.dev. Plano: `/root/.claude/plans/dynamic-swimming-zebra.md`. **FASE 1 ✅** (helper único de dedup, comportamento-neutro).
