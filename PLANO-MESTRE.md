...
0. **Épico Dashboard iFood — FASE 3** (T6): cruzamentos no `RadarReal.jsx` (Balde 2, só frontend — sem migration, sem deploy) — receita líquida, carga total iFood, taxa de cancelamento %, R$ perdido em cancelamentos, gap até o piso Super 4,5; cada KPI guardado por `!= null` (some, não zera, quando falta a fonte). Plano: `/root/.claude/plans/dynamic-swimming-zebra.md`. **FASE 1 ✅** (helper único de dedup, comportamento-neutro) · **FASE 2 ✅** (sessão 48 — parser `processar-fontes.ts` enriquecido: 7 famílias de métricas aditivas via EAV + fix `data_ref` de Operação; merge-first p/ driblar a corrida deploy-vs-reset; deploy + validação no fechamento da sessão).
...
- [x] **FASE 1 — refactor do dedup → helper único `lerMetricas`** ✅ (comportamento-neutro, CAMINHO CRÍTICO): 2 helpers (`trigger/_shared/radar-metricas.ts` + `src/lib/radar-metricas.js`); os 4 call-sites (`RadarReal.jsx`, `agente-analise.ts`, `analise-loja/processar.ts`, `diagnostico-semanal.ts`) trocaram o bloco inline pela chamada **sem `periodo`** → diff de comportamento NULO (verificado por review LOCAL: "all four call-sites reproduce the prior inline behavior exactly"). + guarda anti-vazamento `selectEfetivo` (garante `loja_id` no select quando o filtro client-side `incluirSemLoja` roda — no-op nos call-sites atuais, blinda a Fase 4). Deploy Trigger.dev. · [x] FASE 2 — enriquecer parser (sessão 48) · [ ] FASE 3 — cruzamentos no dashboard · [ ] FASE 4 — filtro temporal UI · [ ] FASE 5 — série diária `radar_series` · [ ] FASE 6 — demandas rascunho→aprovação em `tarefas_loja`
- [x] **FASE 2 — enriquecer o parser (Balde 1, aditivo via EAV)** ✅ (sessão 48): `trigger/radar/processar-fontes.ts` ganhou **7 famílias de métricas novas** sem DDL (EAV absorve) e com o guard `reqCol` fail-loud preservado — **Conciliação:** `conciliacao_repasse_liquido` (entrada−taxas+subsídios) + `conciliacao_lancamentos.metadata.por_tipo` (estorno/antecipação/ajuste); **Cancelamentos:** `cancelamentos_contestaveis_qtd`; **Cardápio:** `cardapio_itens_sem_giro` (cauda-longa de baixo giro); **Operação:** `operacao_cancelamentos_super_qtd` (contagem absoluta) + `operacao_metas` (Meta REAL da planilha, com `valor_texto` — não mais hardcoded); **Logística:** `logistica_pedidos.metadata.por_turno`+`por_servico` (iFood vs própria). + 2 fixes (commit `9216fd2`): `data_ref` de Operação ancorada no período REAL (era a data de processamento) e `operacao_metas` com `valor_texto`. Diff +44/−3 só no parser (`b398509`+`9216fd2`); branch `wandson/dashboard-ifood-fase2`. **Merge-first** p/ driblar a corrida deploy-vs-reset (o `git reset --hard origin/main` da rotina de bridge-restart clobberou um checkout detached mid-build → o deploy `20260614.21` saiu SEM a Fase 2; provado por SQL bruto). Deploy + validação empírica (o gate) no fechamento da sessão. **Vendas (horário de pico/formas de pagamento/melhor dia) ficou para depois** — exige inspecionar `.xlsx` real (anti-padrão P1, Balde 3 deferido).
