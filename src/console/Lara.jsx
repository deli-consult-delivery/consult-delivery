...
function Field({ label, children }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      <label style={{ fontSize: 12, color: 'var(--tx2)', fontWeight: 600 }}>{label}</label>
      {children}
    </div>
  );
}
...
const inputStyle = {
  ...
  background: 'var(--panel)', color: 'var(--tx)', fontSize: 13,
  ...
  return (
    <button
      ...
      style={{
        ...
        background: off ? 'var(--bg)' : 'var(--red)',
        color: off ? 'var(--tx2)' : '#fff',
...
function CopyBtn({ text }) {
  ...
  return (
    <button onClick={copy} title="Copiar" style={{
      ...
      background: 'var(--panel)', color: done ? '#22c55e' : 'var(--tx2)',
...
function UrgBadge({ u }) {
  ...
  return (
    <span style={{
      ...
      background: map[u] || 'var(--bg)', color: '#fff', textTransform: 'uppercase',
    }}>{u}</span>
  );
}
...
function Section({ title, children }) {
  return (
    <div style={{ marginBottom: 20 }}>
      <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--tx2)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 10 }}>{title}</div>
      {children}
    </div>
  );
}
...
function ResultPesquisa({ data }) {
  ...
  return (
    <div style={{ marginTop: 20, display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ padding: 16, background: 'var(--panel)', borderRadius: 10, borderLeft: '3px solid var(--red)' }}>
        <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 4 }}>{data.loja_nome}</div>
        <div style={{ fontSize: 13, color: 'var(--tx2)', lineHeight: 1.6 }}>{data.resumo_executivo}</div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <div style={{ padding: 12, background: 'var(--panel)', borderRadius: 8 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--tx2)', marginBottom: 8 }}>IFOOD</div>
          {data.ifood.nota != null && <div style={{ fontSize: 13 }}>⭐ {data.ifood.nota} ({data.ifood.avaliacoes} avaliações)</div>}
          {data.ifood.ticket_medio && <div style={{ fontSize: 13, color: 'var(--tx2)' }}>Ticket: {data.ifood.ticket_medio}</div>}
          {data.ifood.categorias?.map(c => <span key={c} style={{ display: 'inline-block', fontSize: 11, padding: '2px 8px', background: 'var(--bg)', borderRadius: 10, margin: '2px 2px 0 0' }}>{c}</span>)}
        </div>
        <div style={{ padding: 12, background: 'var(--panel)', borderRadius: 8 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--tx2)', marginBottom: 8 }}>INSTAGRAM</div>
          {data.instagram.handle ? <>
            <div style={{ fontSize: 13 }}>{data.instagram.handle}</div>
            {data.instagram.frequencia && <div style={{ fontSize: 12, color: 'var(--tx2)' }}>{data.instagram.frequencia}</div>}
            {data.instagram.estilo && <div style={{ fontSize: 12, color: 'var(--tx2)' }}>{data.instagram.estilo}</div>}
          </> : <div style={{ fontSize: 12, color: 'var(--tx2)' }}>Não encontrado</div>}
        </div>
      </div>
      ...
      {data.oportunidades?.length > 0 && (
        <Section title="Oportunidades">
          {data.oportunidades.map((o, i) => (
            <div key={i} style={{ fontSize: 13, padding: '8px 12px', background: 'var(--panel)', borderRadius: 8, marginBottom: 6, borderLeft: '3px solid #22c55e' }}>{o}</div>
          ))}
        </Section>
      )}
      ...
      {data.concorrentes?.length > 0 && (
        <Section title="Concorrentes">
          {data.concorrentes.map((c, i) => (
            <div key={i} style={{ fontSize: 13, padding: '8px 12px', background: 'var(--panel)', borderRadius: 8, marginBottom: 6 }}>
              <span style={{ fontWeight: 600 }}>{c.nome}</span>
              <span style={{ color: 'var(--tx2)' }}> — {c.diferencial}</span>
            </div>
          ))}
        </Section>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        {data.cardapio?.destaques?.length > 0 && (
          <Section title="Destaques do cardápio">
            {data.cardapio.destaques.map((d, i) => <div key={i} style={{ fontSize: 13, color: 'var(--tx2)' }}>• {d}</div>)}
          </Section>
        )}
        {data.ifood?.diferenciais?.length > 0 && (
          <Section title="Diferenciais">
            {data.ifood.diferenciais.map((d, i) => <div key={i} style={{ fontSize: 13, color: 'var(--tx2)' }}>• {d}</div>)}
          </Section>
        )}
      </div>
    </div>
  ...
  return (
    <div style={{ marginTop: 20, display: 'flex', flexDirection: 'column', gap: 16 }}>
      {data.variacoes?.map((v, i) => (
        <div key={i} style={{ padding: 16, background: 'var(--panel)', borderRadius: 10, border: '1px solid var(--line)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
            <div style={{ fontSize: 13, fontWeight: 700 }}>{v.titulo}</div>
            <CopyBtn text={v.conteudo + (v.cta ? `\n\n${v.cta}` : '')} />
          </div>
          <div style={{ fontSize: 13, whiteSpace: 'pre-wrap', lineHeight: 1.6, color: 'var(--tx)', marginBottom: 10 }}>{v.conteudo}</div>
          {v.cta && <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--red)' }}>👉 {v.cta}</div>}
          {v.observacoes && <div style={{ fontSize: 11, color: 'var(--tx2)', marginTop: 8, fontStyle: 'italic' }}>💡 {v.observacoes}</div>}
        </div>
      ))}
      {data.dicas_uso?.length > 0 && (
        <div style={{ padding: 12, background: 'var(--panel)', borderRadius: 8 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--tx2)', marginBottom: 8 }}>DICAS DE USO</div>
          {data.dicas_uso.map((d, i) => <div key={i} style={{ fontSize: 12, color: 'var(--tx2)', marginBottom: 4 }}>• {d}</div>)}
        </div>
      )}
    </div>
  );
...
function ResultTendencia({ data }) {
  ...
  return (
    <div style={{ marginTop: 20, display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ padding: 14, background: 'var(--panel)', borderRadius: 10, borderLeft: '3px solid var(--red)' }}>
        <div style={{ fontSize: 13, color: 'var(--tx2)', lineHeight: 1.6 }}>{data.resumo}</div>
      </div>

      {data.tendencias?.map((t, i) => (
        <div key={i} style={{ padding: 14, background: 'var(--panel)', borderRadius: 10, border: '1px solid var(--line)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
            <div style={{ fontSize: 13, fontWeight: 700 }}>{t.titulo}</div>
            <UrgBadge u={t.urgencia} />
          </div>
          <div style={{ fontSize: 13, color: 'var(--tx2)', marginBottom: 8 }}>{t.descricao}</div>
          <div style={{ fontSize: 12, color: 'var(--tx)', background: 'var(--bg)', padding: '8px 10px', borderRadius: 6 }}>
      ...
      {data.alertas?.length > 0 && (
        <Section title="Alertas">
          {data.alertas.map((a, i) => (
            <div key={i} style={{ fontSize: 13, padding: '8px 12px', background: 'var(--panel)', borderRadius: 8, marginBottom: 6, borderLeft: '3px solid #f59e0b' }}>{a}</div>
          ))}
        </Section>
      )}
    </div>
...
function RunCard({ run }) {
  ...
  return (
    <div style={{ padding: 12, background: 'var(--panel)', borderRadius: 8, border: '1px solid var(--line)', marginBottom: 8 }}>
      <div
      ...
      >
        ...
        <span style={{ fontSize: 12, color: 'var(--tx2)' }}>{expanded ? '▲' : '▼'}</span>
      </div>
      {expanded && (
        <div style={{ marginTop: 12, borderTop: '1px solid var(--line)', paddingTop: 12 }}>
          {out.resumo_executivo && <p style={{ fontSize: 13, color: 'var(--tx2)', margin: 0 }}>{out.resumo_executivo}</p>}
          {out.resumo && <p style={{ fontSize: 13, color: 'var(--tx2)', margin: 0 }}>{out.resumo}</p>}
          {out.variacoes?.map((v, i) => (
            <div key={i} style={{ marginTop: 8 }}>
              <div style={{ fontSize: 12, fontWeight: 700 }}>{v.titulo}</div>
              <div style={{ fontSize: 12, color: 'var(--tx2)', whiteSpace: 'pre-wrap', marginTop: 4 }}>{v.conteudo}</div>
            </div>
          ))}
        </div>
      )}
...
export default function Lara({ tenantDbId, userId }) {
  ...
  async function runTask(taskId, payload, tabId) {
    ...
    try {
      const res = await fetch(`${BRIDGE}/agents/${taskId}/run`, {
        method: 'POST',
        headers: {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', maxWidth: 840, margin: '0 auto', padding: '24px 16px 0' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: 'var(--tx2)' }}>CRM &amp; Marketing — food service</div>
        </div>
        ...
        {lojas.length > 0 && (
          <select
            ...
            style={{ marginLeft: 'auto', padding: '6px 10px', borderRadius: 8, border: '1px solid var(--line)', background: 'var(--panel)', color: 'var(--tx)', fontSize: 13 }}
          >
      ...
      <div style={{ display: 'flex', gap: 2, marginBottom: 24, borderBottom: '1px solid var(--line)', paddingBottom: 0 }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{
            ...
            color: tab === t.id ? 'var(--red)' : 'var(--tx2)',
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        {tab === 'rascunhos' && (
          <div>
            {drafts.length === 0 ? (
              <div style={{ textAlign: 'center', color: 'var(--tx2)', fontSize: 13, padding: '40px 0' }}>
              ...
              drafts.map(d => {
                ...
                return (
                  <div key={d.id} style={{
                    padding: 14, background: 'var(--panel)', borderRadius: 10,
                    border: `1px solid ${isPending ? 'var(--line)' : isApproved ? '#22c55e44' : '#ef444444'}`,
                    marginBottom: 10,
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                      <div>
                        ...
                        {tipoMeta && <div style={{ fontSize: 11, color: 'var(--tx2)', marginTop: 2 }}>{tipoMeta.replace(/_/g, ' ')} · {objetivo}</div>}
                        <div style={{ fontSize: 11, color: 'var(--tx2)' }}>{new Date(d.created_at).toLocaleString('pt-BR')}</div>
                      </div>
                      <span style={{
