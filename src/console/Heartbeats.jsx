...
function HeartbeatModal({ heartbeat, onClose, onSaved, bridgeHeaders }) {
  ...
  async function handleSave() {
    ...
    setError('');
    try {
      const url    = isEdit ? `${BRIDGE}/api/heartbeats/${heartbeat.id}` : `${BRIDGE}/api/heartbeats`;
...
function RunsModal({ heartbeat, onClose, bridgeHeaders }) {
  ...
  useEffect(() => {
    fetch(`${BRIDGE}/api/heartbeats/${heartbeat.id}/runs?limit=20`, { headers: bridgeHeaders })
      .then(r => r.json())
...
// ── Heartbeats — Console v2 nativo ─────────────────────────────────────────
export default function Heartbeats({ tenantDbId, userId, onNavigate }) {
  ...
  const loadHeartbeats = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const r = await fetch(`${BRIDGE}/api/heartbeats`, { headers });
  ...
  async function handleToggle(hb) {
    try {
      const r = await fetch(`${BRIDGE}/api/heartbeats/${hb.id}`, {
        method: 'PATCH',
  ...
  async function handleTrigger(hb) {
    ...
    setTrigResult(null);
    try {
      const r = await fetch(`${BRIDGE}/api/heartbeats/${hb.id}/trigger`, {
        method: 'POST',
        headers,
      });
  ...
  async function handleDelete(hb) {
    ...
    try {
      await fetch(`${BRIDGE}/api/heartbeats/${hb.id}`, { method: 'DELETE', headers });
