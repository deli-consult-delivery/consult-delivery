...
/* ─── Main ────────────────────────────────────────────── */
export default function Grupos({ tenantDbId, userId }) {
  const [tab, setTab] = useState('whatsapp');

  return (
    <div style={{ padding: 32, maxWidth: 1200, margin: '0 auto' }}>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ color: 'var(--tx)' }}>Grupos</h1>
        <p style={{ color: 'var(--tx2)' }}>Grupos WhatsApp dos restaurantes e canais internos da equipe</p>
      </div>
      ...
      {tab === 'whatsapp' && <TabWhatsApp tenantDbId={tenantDbId} />}
...
/* ─── Tab WhatsApp Groups ─────────────────────────────── */
function TabWhatsApp({ tenantDbId }) {
    ...
    <div>
      ...
      {toast && (
        <div style={{
          ...
          color: 'var(--tx)', boxShadow: '0 8px 24px rgba(0,0,0,0.2)',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20, padding: 16, background: 'var(--white)', borderRadius: 'var(--r-md)', border: '1px solid var(--g-200)' }}>
        ...
        <button className="cv2-btn sec" onClick={syncFromWA} disabled={syncing || !selInstance} style={{ fontSize: 13 }}>
          <Icon name="refresh" size={13} style={{ animation: syncing ? 'spin 0.8s linear infinite' : 'none' }} />
          {syncing ? 'Sincronizando...' : 'Sincronizar do WA'}
        </button>
        <button className="cv2-btn" onClick={() => setShowCreate(true)} disabled={!selInstance} style={{ fontSize: 13 }}>
    ...
    <div className="cv2-card" style={{ padding: 0, overflow: 'hidden' }}>
      ...
      <div style={{ padding: 18 }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          <button className="cv2-btn sec" style={{ fontSize: 12, padding: '6px 10px', flex: 1 }} onClick={onMembers}>
            <Icon name="users" size={12} /> Participantes
          </button>
          {hasWA && (
            <button className="cv2-btn sec" style={{ fontSize: 12, padding: '6px 10px', flex: 1 }} onClick={onBroadcast}>
              <Icon name="send" size={12} /> Mensagem
            </button>
          )}
          <button className="cv2-btn sec" style={{ fontSize: 12, padding: '6px 10px' }} onClick={onEdit} title="Editar">
            <Icon name="edit" size={13} />
          </button>
          <button className="cv2-btn sec" style={{ fontSize: 12, padding: '6px 10px', color: 'var(--red)' }} onClick={onDelete} title="Excluir">
      ...
      <div className="modal-mobile" style={{ width: 460 }}>
        ...
        <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', paddingTop: 4 }}>
            ...
            <button className="cv2-btn" onClick={handleSave} disabled={saving || !form.group_name.trim()}>
      ...
      <div className="modal-mobile" style={{ width: 440 }}>
        ...
        <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
            ...
            <button className="cv2-btn" onClick={handleSend} disabled={sending || !text.trim()}>
            ...
            <>
              ...
              <GField label="Adicionar participante">
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  />
                  <button className="cv2-btn" onClick={handleAdd} disabled={adding} style={{ flexShrink: 0 }}>
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 20 }}>
        <button className="cv2-btn" onClick={() => setShowCreate(true)}>
      ...
      {channels.length === 0 ? (
        <div style={{ textAlign: 'center', padding: 60, color: 'var(--g-400)', fontSize: 14 }}>
          ...
          <br />
          <button className="cv2-btn sec" style={{ marginTop: 16 }} onClick={() => setShowCreate(true)}>Criar primeiro canal</button>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {channels.map(c => {
            ...
            return (
              <div key={c.id} className="cv2-card" style={{ padding: 18, display: 'flex', alignItems: 'center', gap: 14 }}>
                <div style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    {c.is_global && <span className="cv2-bdg" style={{ fontSize: 10 }}>Global</span>}
                  </div>
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  <button className="cv2-btn sec" style={{ fontSize: 12 }} onClick={() => setManageChan(c)}>
                    <Icon name="users" size={12} /> Membros
                  </button>
                  <button className="cv2-btn sec" style={{ fontSize: 12 }} onClick={() => setEditChan(c)}>
                    <Icon name="edit" size={13} />
                  </button>
                  <button className="cv2-btn sec" style={{ fontSize: 12, color: 'var(--red)' }} onClick={() => handleDelete(c.id)}>
                    <Icon name="trash" size={13} />
                  </button>
                </div>
              </div>
    ...
    <ModalOverlay onClose={onClose}>
      <div className="modal-mobile" style={{ width: 440 }}>
        ...
        <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
            <button className="cv2-btn sec" onClick={onClose}>Cancelar</button>
            <button className="cv2-btn" onClick={handleSave} disabled={saving || !form.name.trim()}>
      ...
      <div className="modal-mobile" style={{ width: 380 }}>
        ...
        <div style={{ padding: 24 }}>
              ...
              {members.map(m => {
                ...
                return (
                  <label key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', padding: '8px 10px', borderRadius: 8, background: active ? 'var(--red-soft)' : 'var(--g-50)', border: `1px solid ${active ? 'var(--red)' : 'var(--g-200)'}` }}>
                    ...
                    <div style={{ width: 30, height: 30, borderRadius: '50%', background: active ? 'var(--red)' : 'var(--g-200)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: active ? 'var(--tx)' : 'var(--g-700)' }}>
                      {(m.full_name || m.email || '?').split(' ').map(w => w[0]).join('').toUpperCase().slice(0, 2)}
                    </div>
                    <div style={{ flex: 1 }}>
                    ...
                    {active && <span className="cv2-bdg" style={{ fontSize: 10 }}>Membro</span>}
                  </label>
                );
              })}
            </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 16 }}>
            <button className="cv2-btn" onClick={onClose}>Fechar</button>
          </div>
        </div>
      </div>
    </ModalOverlay>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
        ...
        />
        <button className="cv2-btn sec" onClick={addManual} style={{ flexShrink: 0, padding: '0 12px' }}>
...
function ModalHeader({ title, onClose }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 24px 0' }}>
      ...
      <button className="cv2-btn sec" onClick={onClose}><Icon name="x" size={16} /></button>
    </div>
  );
}
