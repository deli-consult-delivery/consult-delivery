...
function MemoryModal({ memory, onClose, onSaved, bridgeHeaders }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const url    = isEdit ? `${BRIDGE}/api/memories/${memory.id}` : `${BRIDGE}/api/memories`;
  ...
  const labelSt = { display: 'block', fontSize: 12, color: 'var(--tx2)', marginBottom: 5 };
  const inputSt = {
      ...
      <div style={box}>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ marginBottom: 16 }}>
            <label style={labelSt}>Agente</label>
            <select style={selectSt} value={form.agent_slug} onChange={e => set('agent_slug', e.target.value)} disabled={isEdit}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <select style={selectSt} value={form.kind} onChange={e => set('kind', e.target.value)}>
          ...
          <div style={{ marginBottom: 16 }}>
            <label style={labelSt}>Conteúdo</label>
            <textarea
              style={{ ...inputSt, minHeight: 100, resize: 'vertical', fontFamily: 'inherit' }}
              value={form.content}
          ...
          <div style={{ marginBottom: 16 }}>
            <label style={labelSt}>Importância: {form.importance}/10</label>
            <input
          ...
          <div style={{ marginBottom: 20 }}>
            <label style={labelSt}>Expira em (opcional)</label>
            <input
...
// ── Tela principal ──────────────────────────────────────────────────────────
export default function Memorias({ tenantDbId, userId }) {
  ...
  async function handleDelete(id) {
    ...
    try {
      const r = await fetch(`${BRIDGE}/api/memories/${id}`, {
  ...
  async function handleWipe(slug) {
    try {
      const r = await fetch(`${BRIDGE}/api/memories/agent/${slug}`, {
        method: 'DELETE', headers: bridgeHeaders,
      });
      if (!r.ok) { const d = await r.json(); throw new Error(d.error || r.statusText); }
      setConfirmWipe(null);
  ...
  return (
    <div>
      ...
      {filtered.map(m => {
        ...
        return (
          <div key={m.id} className="cv2-card" style={{ marginBottom: 10 }}>
