...
import NovoContratoModalV2 from './NovoContratoModalV2.jsx';
...
function PagtoStatusBadge({ status }) {
  if (!status) return <span style={{ fontSize: 11, color: 'var(--tx2)' }}>—</span>;
...
export default function Contratos({ tenantDbId, userId }) {
  ...
  return (
    <div style={{ padding: '24px 28px', maxWidth: 960, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: 'var(--tx)' }}>
            Contratos
          </h1>
          <p style={{ margin: '4px 0 0', fontSize: 13, color: 'var(--tx2)' }}>
      ...
      {/* Content */}
      {loading ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: 'var(--tx2)', padding: 40, justifyContent: 'center' }}>
          <Spinner size={18} /> Carregando contratos…
        </div>
      ) : contratos.length === 0 ? (
        <div className="cv2-card" style={{ borderRadius: 12, padding: '56px 24px', textAlign: 'center' }}>
          <div style={{ fontSize: 36, marginBottom: 12 }}>📄</div>
          <p style={{ margin: 0, color: 'var(--tx2)', fontSize: 14 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {contratos.map(c => (
            <div key={c.id} className="cv2-card" style={{
              ...
              <div style={{ textAlign: 'right', minWidth: 100 }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: COLOR }}>
                  {fmtBRL(c.valor_mensal)}<span style={{ fontSize: 10, fontWeight: 400, color: 'var(--tx2)' }}>/mês</span>
                </div>
                {c.valor_setup > 0 && (
      ...
      {toast && (
        <div style={{
          ...
          color: 'var(--tx)', fontSize: 13, fontWeight: 500,
