...
export default function Templates({ tenantDbId, userId }) {
  ...
  async function criar() {
    ...
    setSalvando(true);
    const { error } = await supabase.from('templates').insert({ tenant_id: tenantDbId, tipo, nome: nome.trim(), conteudo, created_by: userId ?? null });
    setSalvando(false);
    if (error) { setErro(error.message); return; }
    setNome(''); setConteudo('');
    await carregar();
  ...
  return (
    <div>
      ...
      <div className="cv2-card" style={{ maxWidth: 620 }}>
        ...
        <div style={{ marginTop: 12 }}><button className="cv2-btn" disabled={salvando} onClick={criar}>{salvando ? 'Salvando…' : 'Criar template'}</button></div>
      </div>
      {rows && rows.map(r => (
