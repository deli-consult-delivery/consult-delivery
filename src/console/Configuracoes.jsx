...
import CustomFieldsManager from '../screens/Settings/CustomFieldsManager.jsx';

const BRIDGE = import.meta.env.VITE_BRIDGE_URL || 'https://bridge.consultdelivery.com.br';

// ============================================================
// Console v2 — Configurações (migração de SettingsScreen)
// Props: tenantDbId, userId, onTenantChange
// ============================================================

export default function Configuracoes({ tenantDbId, userId, onTenantChange }) {
  ...
  return (
    <div className="route-enter" style={{ padding: '28px 32px 56px', maxWidth: 1480, margin: '0 auto' }}>
      ...
      <div className="set-layout">
        <nav className="cv2-card set-nav" style={{ padding: 0 }}>
          {sections.map(s => (
            <button
              key={s.id}
...
const WorkspaceSettings = () => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
    <SettingsCard title="Identidade do espaço">
      <div style={{ display: 'grid', gridTemplateColumns: '160px 1fr', gap: 24, alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <button className="cv2-btn sec" style={{ alignSelf: 'flex-start' }}>
          ...
          <FormRow label="Identificador (URL)">
            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              <span style={{ fontSize: 13, color: 'var(--tx2)' }}>consultdelivery.com.br/</span>
      ...
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
        {[
          ...
          <div key={c.label} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 11, color: 'var(--tx2)', textTransform: 'uppercase', fontWeight: 600 }}>{c.label}</div>
              <div style={{ fontSize: 13, color: 'var(--tx)', fontWeight: 700 }}>{c.color}</div>
            </div>
          </div>
        ))}
      </div>
    ...
    <SettingsCard title="Horário de atendimento" sub="Define quando humanos respondem; fora disso, IA assume com aviso">
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {['Segunda', 'Terça', 'Quarta', 'Quinta', 'Sexta', 'Sábado', 'Domingo'].map((d, i) => (
          <div key={d} style={{ display: 'grid', gridTemplateColumns: '120px 1fr 100px 14px 100px', gap: 12, alignItems: 'center', padding: 8, borderRadius: 6, background: i % 2 === 0 ? 'var(--panel)' : 'transparent' }}>
            ...
            <span style={{ textAlign: 'center', color: 'var(--tx2)' }}>—</span>
    ...
    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
      <button className="cv2-btn sec">Cancelar</button>
      <button className="cv2-btn">Salvar alterações</button>
    </div>
  </div>
);
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(28,27,26,0.45)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: 'var(--panel)', border: '1px solid var(--line)', borderRadius: 14, padding: 28, boxShadow: '0 12px 40px rgba(0,0,0,0.18)', width: 380, position: 'relative' }}>
        ...
        <input
          ...
          style={{ width: '100%', padding: '10px 12px', background: 'var(--panel)', border: '1px solid var(--line)', borderRadius: 8, color: 'var(--tx)', fontSize: 14, marginBottom: 16, boxSizing: 'border-box' }}
        />
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(28,27,26,0.45)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: 'var(--panel)', border: '1px solid var(--line)', borderRadius: 14, padding: 28, boxShadow: '0 12px 40px rgba(0,0,0,0.18)', width: 380, position: 'relative' }}>
        ...
        <select value={role} onChange={e => setRole(e.target.value)} style={{ width: '100%', padding: '10px 12px', background: 'var(--panel)', border: '1px solid var(--line)', borderRadius: 8, color: 'var(--tx)', fontSize: 14, marginBottom: 16 }}>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(28,27,26,0.45)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: 'var(--panel)', border: '1px solid var(--line)', borderRadius: 14, padding: 28, boxShadow: '0 12px 40px rgba(0,0,0,0.18)', width: 400, position: 'relative' }}>
        ...
        <input type="email" placeholder="email@empresa.com" value={email} onChange={e => setEmail(e.target.value)} style={{ width: '100%', padding: '10px 12px', background: 'var(--panel)', border: '1px solid var(--line)', borderRadius: 8, color: 'var(--tx)', fontSize: 14, marginBottom: 12, boxSizing: 'border-box' }} />
        <select value={role} onChange={e => setRole(e.target.value)} style={{ width: '100%', padding: '10px 12px', background: 'var(--panel)', border: '1px solid var(--line)', borderRadius: 8, color: 'var(--tx)', fontSize: 14, marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          <button className="cv2-btn sec" onClick={onClose}>Cancelar</button>
          <button className="cv2-btn" onClick={handleInvite} disabled={saving}>{saving ? 'Enviando...' : 'Enviar convite'}</button>
        </div>
      </div>
    </div>
  );
      ...
      <div style={{ background: 'var(--panel)', border: '1px solid var(--line)', borderRadius: 14, padding: 28, boxShadow: '0 12px 40px rgba(0,0,0,0.18)', width: 520, maxHeight: '80vh', overflow: 'auto', position: 'relative' }}>
        ...
        {loading ? (
          ...
          Object.entries(groups).map(([group, screens]) => (
            <div key={group}>
              <div style={{ fontSize: 11, color: 'var(--tx2)', textTransform: 'uppercase', letterSpacing: '0.06em', marginTop: 20, marginBottom: 8 }}>{group}</div>
              {screens.map(screen => {
...
const UsersSettings = ({ tenantDbId }) => {
  ...
  return (
    <SettingsCard
      ...
      extra={<button className="cv2-btn" onClick={() => setInviting(true)}><Icon name="plus" size={13} /> Convidar usuário</button>}
    >
      {loading ? (
        <div style={{ padding: 24, textAlign: 'center', color: 'var(--tx2)', fontSize: 13 }}>Carregando...</div>
      ) : (
            ...
            {team.map((u, i) => {
              ...
              return (
                <tr key={u.user_id}>
                  ...
                  <td><span className={`cv2-bdg ${ROLE_BADGE[u.role] || 'badge-gray'}`}>{ROLE_LABEL[u.role] || u.role}</span></td>
                  <td><span style={{ color: dotColor[status], fontSize: 12, fontWeight: 700 }}>{dotLabel[status]}</span></td>
                  <td style={{ color: 'var(--tx2)', fontSize: 12 }}>{relativeTime(u.last_sign_in_at)}</td>
                  <td>
                    <div style={{ position: 'relative', display: 'inline-block' }}>
                      ...
                      {activeMenu === u.user_id && (
                        <div style={{ position: 'absolute', right: 0, top: '100%', zIndex: 1000, background: 'var(--panel)', border: '1px solid var(--line)', borderRadius: 8, minWidth: 180, boxShadow: '0 8px 24px rgba(0,0,0,0.18)', overflow: 'hidden' }}>
                          <button onClick={() => { setEditingNameMember(u); setActiveMenu(null); }} style={{ display: 'block', width: '100%', padding: '10px 14px', background: 'none', border: 'none', color: 'var(--tx)', fontSize: 13, textAlign: 'left', cursor: 'pointer' }}>Editar nome</button>
                          <button onClick={() => { setEditingMember(u); setActiveMenu(null); }} style={{ display: 'block', width: '100%', padding: '10px 14px', background: 'none', border: 'none', color: 'var(--tx)', fontSize: 13, textAlign: 'left', cursor: 'pointer' }}>Editar permissao</button>
                          <button onClick={() => { setScreenPermsMember(u); setActiveMenu(null); }} style={{ display: 'block', width: '100%', padding: '10px 14px', background: 'none', border: 'none', color: 'var(--tx)', fontSize: 13, textAlign: 'left', cursor: 'pointer' }}>Gerenciar acesso as telas</button>
                          <button onClick={() => { setRemovingMember(u); setActiveMenu(null); }} style={{ display: 'block', width: '100%', padding: '10px 14px', background: 'none', border: 'none', color: 'var(--red)', fontSize: 13, textAlign: 'left', cursor: 'pointer' }}>Remover usuario</button>
                        </div>
                      )}
                    </div>
                  </td>
...
const DepartmentsSettings = () => {
  const depts = [
    ...
    { name: 'Cobranca',    color: '#F59E0B', members: 3,  agent: 'cora'  },
    { name: 'Operacoes',   color: '#8B5CF6', members: 4,  agent: 'max'   },
  ];
  return (
    <SettingsCard title="Departamentos" sub="Cada departamento tem fila própria, agentes e SLA" extra={<button className="cv2-btn"><Icon name="plus" size={13} /> Novo departamento</button>}>
        ...
        {depts.map(d => (
          <div key={d.name} className="dept-card">
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--tx2)', marginTop: 2 }}>{d.members} agentes humanos</div>
            </div>
            <button className="cv2-btn sec"><Icon name="settings" size={14} /></button>
          </div>
        ))}
      </div>
    </SettingsCard>
        ...
        {integrations.map(i => (
          <div key={i.name} className="integration-card">
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
              <div style={{ width: 40, height: 40, borderRadius: 10, background: 'var(--panel)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22 }}>{i.icon}</div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 700, color: 'var(--tx)' }}>{i.name}</div>
                <div style={{ fontSize: 11, color: 'var(--tx2)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{i.desc}</div>
              </div>
              <span className={`cv2-bdg ${i.status === 'connected' ? 'badge-green' : i.status === 'pending' ? 'badge-amber' : 'badge-gray'}`} style={{ fontSize: 10 }}>
                {i.status === 'connected' ? 'ativo' : i.status === 'pending' ? 'pendente' : 'disponível'}
              </span>
            </div>
            {i.stat && (
              <div style={{ fontSize: 12, color: 'var(--tx2)', padding: 8, background: 'var(--panel)', borderRadius: 6, marginBottom: 8 }}>{i.stat}</div>
            )}
            <button className={i.status === 'available' ? 'cv2-btn' : 'cv2-btn sec'} style={{ width: '100%', fontSize: 13 }}>
...
const TemplatesSettings = () => {
  ...
  return (
    <SettingsCard
      ...
      extra={<button className="cv2-btn"><Icon name="plus" size={13} /> Novo template</button>}
    >
          ...
          <div key={t.name} className="template-row">
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                <span style={{ fontFamily: 'var(--font-mono, monospace)', fontWeight: 700, color: 'var(--tx)' }}>{t.name}</span>
                <span className={`cv2-bdg ${t.status === 'aprovado' ? 'badge-green' : t.status === 'em_revisao' ? 'badge-amber' : 'badge-red'}`} style={{ fontSize: 10 }}>{t.status}</span>
                <span style={{ fontSize: 10, color: 'var(--tx2)', textTransform: 'uppercase', letterSpacing: 0.5, fontWeight: 600 }}>{t.cat}</span>
              </div>
              <div style={{ fontSize: 12, color: 'var(--tx2)', fontStyle: 'italic' }}>"{t.sample}"</div>
            </div>
            <div style={{ textAlign: 'right', flexShrink: 0 }}>
              <div style={{ fontSize: 11, color: 'var(--tx2)' }}>usos</div>
              <div style={{ fontSize: 16, fontWeight: 800, color: 'var(--tx)' }}>{t.uses}</div>
            </div>
          </div>
        ))}
      </div>
...
const RulesSettings = () => {
  ...
  return (
    <SettingsCard
      ...
      extra={<button className="cv2-btn"><Icon name="plus" size={13} /> Nova regra</button>}
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {rules.map(r => (
          <div key={r.name} className="rule-row">
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontWeight: 700, color: 'var(--tx)', marginBottom: 4 }}>{r.name}</div>
              <div style={{ fontSize: 12, color: 'var(--tx2)' }}>
                <strong style={{ color: '#B70C00' }}>SE</strong> {r.when} <strong style={{ color: 'var(--tx)' }}>→</strong> <strong style={{ color: '#B70C00' }}>ENTÃO</strong> {r.then}
              </div>
            </div>
            <button className="cv2-btn sec"><Icon name="settings" size={14} /></button>
          </div>
...
const BillingSettings = () => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
    <SettingsCard title="Plano atual">
      <div style={{ display: 'flex', alignItems: 'center', gap: 24, padding: 16, background: 'linear-gradient(135deg, var(--bg), var(--panel))', borderRadius: 12 }}>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 11, fontWeight: 800, color: 'var(--red-light, #FF8A82)', letterSpacing: 1, textTransform: 'uppercase' }}>Plano Pro IA</div>
          <div style={{ fontSize: 28, fontWeight: 800, marginTop: 4, color: 'var(--tx)' }}>R$ 489 <span style={{ fontSize: 14, opacity: 0.7, fontWeight: 400 }}>/mês</span></div>
          <div style={{ fontSize: 12, color: 'var(--tx2)', marginTop: 6 }}>5 superagentes · 10 humanos · 50k mensagens/mês</div>
        </div>
        <button className="cv2-btn">Alterar plano</button>
      </div>
    </SettingsCard>
        ...
        return (
          <div key={u.label} style={{ marginBottom: 14 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 6 }}>
              <span style={{ color: 'var(--tx2)' }}>{u.label}</span>
              <span style={{ fontWeight: 700, color: 'var(--tx)' }}>{u.used.toLocaleString('pt-BR')} / {u.max.toLocaleString('pt-BR')}</span>
            </div>
            <div style={{ height: 8, background: 'var(--panel)', borderRadius: 99 }}>
        ...
        <tbody>
          {['Abril 2026', 'Março 2026', 'Fevereiro 2026', 'Janeiro 2026'].map(m => (
            <tr key={m}>
              ...
              <td><span className="cv2-bdg badge-green">pago</span></td>
              <td><button className="cv2-btn sec" style={{ fontSize: 12 }}>Baixar PDF</button></td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
    ...
    <SettingsCard title="Zona de perigo">
      <div className="danger-row">
        <div>
          <div style={{ fontWeight: 700, color: 'var(--tx)' }}>Excluir espaço de trabalho</div>
          <div style={{ fontSize: 12, color: 'var(--tx2)', marginTop: 4 }}>Esta ação é permanente e remove todos os dados, conversas e clientes.</div>
        </div>
        <button className="cv2-btn sec" style={{ borderColor: '#EF4444', color: '#EF4444' }}>Excluir espaço</button>
      </div>
    </SettingsCard>
  </div>
);
...
const SettingsCard = ({ title, sub, extra, children }) => (
  <div className="cv2-card" style={{ padding: 20 }}>
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16, gap: 16 }}>
      <div>
        <h3 style={{ fontSize: 15, fontWeight: 800, color: 'var(--tx)', margin: 0 }}>{title}</h3>
        {sub && <div style={{ fontSize: 12, color: 'var(--tx2)', marginTop: 4 }}>{sub}</div>}
      </div>
      {extra}
    </div>
    {children}
...
const FormRow = ({ label, children }) => (
  <div>
    <div style={{ fontSize: 11, color: 'var(--tx2)', textTransform: 'uppercase', letterSpacing: 0.5, fontWeight: 600, marginBottom: 6 }}>{label}</div>
    {children}
  </div>
);
...
const ToggleRow = ({ label, sub, on }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
    ...
    <div>
      ...
      <div style={{ fontSize: 11, color: 'var(--tx2)', marginTop: 2 }}>{sub}</div>
    </div>
  </div>
);
