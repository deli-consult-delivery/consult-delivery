...
const STATUS_LABEL = {
  rascunho:              { label: 'Rascunho',          cls: 'cv2-bdg'   },
  aguardando_envio:      { label: 'Ag. Envio',         cls: 'cv2-bdg'   },
  aguardando_aprovacao:  { label: 'Ag. Aprovação',     cls: 'cv2-bdg'   },
  aprovada:              { label: 'Aprovada',           cls: 'cv2-bdg'   },
  rejeitada:             { label: 'Rejeitada',          cls: 'cv2-bdg'   },
  em_execucao:           { label: 'Em Execução',        cls: 'cv2-bdg'   },
  aguardando_validacao:  { label: 'Ag. Validação',     cls: 'cv2-bdg'   },
  concluida:             { label: 'Concluída',          cls: 'cv2-bdg'   },
  cancelada:             { label: 'Cancelada',          cls: 'cv2-bdg'   },
};

const PRIORIDADE_LABEL = {
  quick_win:        { label: 'Quick Win',       cls: 'cv2-bdg'   },
  estrutural:       { label: 'Estrutural',      cls: 'cv2-bdg'   },
  material_cliente: { label: 'Mat. Cliente',    cls: 'cv2-bdg'   },
};
...
export default function TarefasGlobais({ tenantDbId, userId }) {
  ...
  const loadTarefas = useCallback(async (resetPage = false, pageOverride = null) => {
    ...
    try {
      ...
      console.error('[TarefasGlobais] loadTarefas:', e.message);
    } finally {
      setLoading(false);
    }
  ...
  const loadCalendario = useCallback(async () => {
    ...
    try {
      ...
      console.error('[TarefasGlobais] loadCalendario:', e.message);
    } finally {
      setCalLoading(false);
    }
  ...
  async function moveCard(id, newStatus) {
    ...
    try {
      ...
      console.error('[TarefasGlobais] moveCard:', e.message);
      loadTarefas(true);
    }
  }
  ...
  return (
    <div style={{ padding: 24, height:'calc(100vh - 64px)', display:'flex', flexDirection:'column', background:'var(--bg)', overflow:'hidden' }}>

      {/* Header */}
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginBottom: 16 }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 700, color:'var(--tx)', margin: 0 }}>Todas as Tarefas</h1>
          <p style={{ fontSize: 13, color:'var(--tx2)', margin: '2px 0 0' }}>{tarefas.length} tarefas · {ativas} ativas</p>
        </div>
        <div style={{ display:'flex', gap: 8, alignItems:'center' }}>
          <ViewToggle view={view} setView={setView}/>
          <button className="cv2-btn" onClick={() => setShowModal(true)}>
      ...
      <div style={{ display:'flex', gap: 8, marginBottom: 16, flexWrap:'wrap', alignItems:'center' }}>
        ...
        {(filtroLoja||filtroStatus||filtroPrioridade||filtroPrazoDe||filtroPrazoAte) && (
          <button className="cv2-btn sec" style={{ fontSize: 12 }} onClick={() => {
      ...
      <div style={{ flex: 1, overflow:'hidden', position:'relative' }}>
        {loading && tarefas.length === 0 && (
          <div style={{ display:'flex', justifyContent:'center', alignItems:'center', height: '100%', color:'var(--tx2)' }}>
            Carregando...
          </div>
        )}
      ...
      {draggingTask && ghostPos && (
        <div className="cv2-card drag-ghost" style={{ left: ghostPos.x - 130, top: ghostPos.y - 30, position:'fixed', zIndex: 9999, pointerEvents:'none' }}>
          <div style={{ fontSize: 12, fontWeight: 600, color:'var(--tx)' }}>{draggingTask.titulo}</div>
        </div>
      )}
...
function ViewToggle({ view, setView }) {
  ...
  return (
    <div style={{ display:'flex', background:'var(--panel)', borderRadius: 6, padding: 2, gap: 2 }}>
      {views.map(v => (
        <button
          key={v.id}
          ...
          style={{
            ...
            background: view === v.id ? 'var(--panel)' : 'transparent',
            color: view === v.id ? 'var(--tx)' : 'var(--tx2)',
  ...
  return (
    <div style={{ height:'100%', overflowY:'auto' }} className="scroll">
      <table style={{ width:'100%', borderCollapse:'collapse', fontSize: 13 }}>
          ...
          <tr style={{ background:'var(--panel)', position:'sticky', top: 0, zIndex: 1 }}>
            {cols.map(c => (
              <th
                ...
                style={{ padding:'8px 12px', textAlign:'left', fontWeight:700, fontSize:11, color:'var(--tx2)', cursor:'pointer', userSelect:'none', whiteSpace:'nowrap' }}
              >
                {c.label}
        ...
        <tbody>
          {tarefas.length === 0 && (
            <tr><td colSpan={cols.length} style={{ padding: 32, textAlign:'center', color:'var(--tx2)' }}>Nenhuma tarefa encontrada</td></tr>
          )}
          {tarefas.map(t => {
            const s = STATUS_LABEL[t.status] || { label: t.status, cls: 'cv2-bdg' };
            const p = PRIORIDADE_LABEL[t.prioridade] || { label: t.prioridade, cls: 'cv2-bdg' };
            return (
              <tr key={t.id} style={{ borderBottom:'1px solid var(--panel)', background:'var(--bg)' }}
                onMouseEnter={e => e.currentTarget.style.background = 'var(--panel)'}
                onMouseLeave={e => e.currentTarget.style.background = 'var(--bg)'}
              >
                <td style={{ padding:'8px 12px', fontWeight: 500, maxWidth: 320 }}>
                  <div style={{ overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', color:'var(--tx)' }}>{t.titulo}</div>
                </td>
                <td style={{ padding:'8px 12px', color:'var(--tx2)', fontSize: 12 }}>{t.loja_nome || '—'}</td>
                <td style={{ padding:'8px 12px', color:'var(--tx2)', fontSize: 12, textTransform:'capitalize' }}>{t.bloco || '—'}</td>
                <td style={{ padding:'8px 12px' }}><span className={`cv2-bdg ${p.cls}`}>{p.label}</span></td>
                <td style={{ padding:'8px 12px' }}><span className={`cv2-bdg ${s.cls}`}>{s.label}</span></td>
                <td style={{ padding:'8px 12px', color: t.prazo_estimado ? 'var(--tx)' : 'var(--tx2)', fontSize: 12 }}>
                  {t.prazo_estimado || '—'}
                </td>
              </tr>
            );
      ...
      {hasMore && (
        <div style={{ padding: 16, textAlign:'center' }}>
          <button className="cv2-btn sec" onClick={loadMore}>Carregar mais</button>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap: 12, height:'100%', overflow:'hidden' }}>
      {BOARD_COLS.map(col => (
        <div
          key={col.id}
          className={`cv2-card scroll ${hoverCol === col.id ? 'drop-hover' : ''}`}
        ...
        >
          <div style={{ display:'flex', alignItems:'center', gap: 8, padding:'4px 6px 10px', position:'sticky', top: 0, background:'var(--panel)' }}>
            <span style={{ width: 10, height: 10, borderRadius:'50%', background: col.color }}/>
            <span style={{ fontSize: 12, fontWeight: 700, color:'var(--tx)', textTransform:'uppercase', letterSpacing: 0.3 }}>{col.label}</span>
            <span style={{ fontSize: 11, color:'var(--tx2)', background:'var(--bg)', padding:'1px 7px', borderRadius: 9999 }}>{byCol[col.id].length}</span>
          </div>
          <div style={{ display:'flex', flexDirection:'column', gap: 8 }}>
            ...
            {byCol[col.id].length === 0 && (
              <div style={{ padding: 16, textAlign:'center', fontSize: 12, color:'var(--tx2)', border:'2px dashed var(--panel)', borderRadius: 6 }}>
                Arraste aqui
              </div>
            )}
          </div>
...
function BoardCard({ task, isDragging, onDragStart, onDrag, onDragEnd }) {
  const p = PRIORIDADE_LABEL[task.prioridade] || { label: task.prioridade, cls: 'cv2-bdg' };
  return (
    <div
      className={`cv2-card ${isDragging ? 'dragging' : ''}`}
      draggable
      onDragStart={onDragStart}
      onDrag={onDrag}
      onDragEnd={onDragEnd}
    >
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom: 6 }}>
        <span className={`cv2-bdg ${p.cls}`} style={{ fontSize: 10 }}>{p.label}</span>
        {task.loja_nome && <span style={{ fontSize: 10, color:'var(--tx2)' }}>{task.loja_nome}</span>}
      </div>
      <div style={{ fontSize: 12, fontWeight: 600, color:'var(--tx)', lineHeight: 1.35, marginBottom: 6 }}>{task.titulo}</div>
      {task.prazo_estimado && (
  ...
  return (
    <div style={{ height:'100%', display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap: 16, marginBottom: 12 }}>
        ...
        <span style={{ fontWeight: 700, fontSize: 15, minWidth: 160, textAlign:'center', color:'var(--tx)' }}>
        ...
        {calLoading && <span style={{ fontSize: 12, color:'var(--tx2)' }}>Carregando...</span>}
      </div>

      {/* Header dias */}
      <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap: 2, marginBottom: 2 }}>
        {['Dom','Seg','Ter','Qua','Qui','Sex','Sáb'].map(d => (
          <div key={d} style={{ textAlign:'center', fontSize: 11, fontWeight: 700, color:'var(--tx2)', padding:'4px 0' }}>{d}</div>
        ))}
      </div>
        ...
        {cells.map((d, i) => {
          ...
          return (
            <div
              ...
              style={{
                minHeight: 70, padding: 6, borderRadius: 6, border:'1px solid var(--panel)',
                background: isToday(d) ? 'rgba(183,12,0,0.05)' : 'var(--bg)',
                cursor: hasItems ? 'pointer' : 'default',
                position:'relative',
              }}
              onMouseEnter={e => { if (hasItems) e.currentTarget.style.background = 'var(--panel)'; }}
              onMouseLeave={e => { e.currentTarget.style.background = isToday(d) ? 'rgba(183,12,0,0.05)' : 'var(--bg)'; }}
            >
              <div style={{
                fontSize: 12, fontWeight: isToday(d) ? 700 : 500,
                color: isToday(d) ? 'var(--red)' : 'var(--tx)',
                marginBottom: 4,
              }}>{d}</div>
              ...
              {items.length > 3 && (
                <div style={{ fontSize: 10, color:'var(--tx2)' }}>+{items.length - 3} mais</div>
              )}
            </div>
          );
        })}
      ...
      {calModal && (
        <div style={{ position:'fixed', inset: 0, background:'rgba(0,0,0,0.4)', zIndex: 1000, display:'flex', alignItems:'center', justifyContent:'center' }}
          ...
          <div style={{ background:'var(--panel)', borderRadius: 10, padding: 24, minWidth: 320, maxWidth: 500, maxHeight:'70vh', overflowY:'auto' }}
            onClick={e => e.stopPropagation()}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom: 16 }}>
              <h3 style={{ fontWeight: 700, fontSize: 15, color:'var(--tx)' }}>Tarefas — {calModal.date}</h3>
            ...
            {calModal.tarefas.map(t => {
              ...
              const p = PRIORIDADE_LABEL[t.prioridade] || { label: t.prioridade, cls: 'cv2-bdg' };
              return (
                <div key={t.id} style={{ padding:'10px 0', borderBottom:'1px solid var(--panel)' }}>
                  <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 4, color:'var(--tx)' }}>{t.titulo}</div>
                  <div style={{ display:'flex', gap: 6 }}>
                    <span className={`cv2-bdg ${s.cls}`}>{s.label}</span>
                    <span className={`cv2-bdg ${p.cls}`}>{p.label}</span>
                    {t.loja_nome && <span style={{ fontSize: 11, color:'var(--tx2)' }}>{t.loja_nome}</span>}
                  </div>
                </div>
              );
            })}
...
function NovaTarefaModal({ lojas, onClose, onCreated }) {
  ...
  return (
    <div style={{ position:'fixed', inset: 0, background:'rgba(0,0,0,0.5)', zIndex: 1000, display:'flex', alignItems:'center', justifyContent:'center' }}
      onClick={onClose}>
      <div style={{ background:'var(--panel)', borderRadius: 12, padding: 28, width: 480, maxWidth:'95vw' }}
        onClick={e => e.stopPropagation()}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom: 20 }}>
          <h2 style={{ fontWeight: 700, fontSize: 16, color:'var(--tx)' }}>Nova Tarefa</h2>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Field label="Título">
            <input type="text" className="cv2-input" style={{ width:'100%' }}
          ...
          <Field label="O que será feito">
            <textarea className="cv2-input" style={{ width:'100%', resize:'vertical', minHeight: 80 }}
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap: 12 }}>
            ...
            <Field label="Prazo estimado">
              <input type="date" className="cv2-input" style={{ width:'100%' }}
          ...
          <div style={{ display:'flex', gap: 8, justifyContent:'flex-end', marginTop: 8 }}>
            ...
            <button type="submit" className="cv2-btn" disabled={saving}>{saving ? 'Salvando...' : 'Criar Tarefa'}</button>
          </div>
        </form>
      </div>
    </div>
...
function Field({ label, children }) {
  return (
    <div style={{ marginBottom: 14 }}>
      <label style={{ display:'block', fontSize: 11, fontWeight: 700, color:'var(--tx2)', marginBottom: 4 }}>{label}</label>
      {children}
    </div>
  );
}
