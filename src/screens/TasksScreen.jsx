// ═══════════════════════════════════════════════════════════════
// KANBAN SCREEN — with real drag-and-drop
// ═══════════════════════════════════════════════════════════════

import { useState as uSK, useEffect as uEK, useRef as uRK, useMemo as uMK } from 'react';
import Icon from '../components/Icon.jsx';
import AgentAvatar from '../components/AgentAvatar.jsx';
import UserAvatar from '../components/UserAvatar.jsx';
import { TASKS, AGENTS } from '../data.js';
...
const KanbanScreen = ({ tenant, tenantDbId }) => {
  const initial = TASKS[tenant] || [];
  const [tasks, setTasks] = uSK(initial);
  const [dragId, setDragId] = uSK(null);
  const [hoverCol, setHoverCol] = uSK(null);
  const [ghostPos, setGhostPos] = uSK(null);
  const ghostRef = uRK(null);

  uEK(() => { setTasks(TASKS[tenant] || []); }, [tenant]);
    ...
    <div className="route-enter" style={{ padding: 32, height:'calc(100vh - 64px)', display:'flex', flexDirection:'column', background:'var(--g-50)' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap: 16, flex: 1, overflow:'hidden' }}>
        {COLS.map(col => (
          ...
          >
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding: '2px 6px 12px', position:'sticky', top: 0 }}>
...
const TaskCard = ({ task, isDragging, onDragStart, onDrag, onDragEnd }) => (
  <div
  ...
  return (
    <>
      <div style={{
        ...
        marginBottom: 10, width: task.priority === 'high' ? '100%' : task.priority === 'med' ? '60%' : '30%',
      }}/>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom: 8 }}>
        ...
        {task.agent && (
          <div style={{ display:'flex', alignItems:'center', gap: 4 }}>
            ...
            <span style={{ fontSize: 10, fontWeight: 600, color: AGENTS.find(a => a.id === task.agent).color }}>
