...
/* ─── Main ────────────────────────────────────────────── */
export default function KanbanScreen({ tenant, tenantDbId }) {
  ...
  async function saveTask(data) {
    if (editTarget) {
      ...
      setTasks(ts => ts.map(t => t.id === editTarget.id ? { ...t, title: data.title, desc: data.desc, col: data.col, priority: data.priority, due: data.due, agent: data.agent || null, resolucao: data.resolucao } : t));
        ...
        <div className="btn-wrap" style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <div ref={filtRef} style={{ position: 'relative' }}>
            <button className="btn-secondary"
              ...
              style={{
                background: filterPriority !== 'all' || filterAI || filterToday || filterIfood ? 'var(--red-soft)' : undefined,
                borderColor: filterPriority !== 'all' || filterAI || filterToday || filterIfood ? 'var(--red)' : undefined,
                color: filterPriority !== 'all' || filterAI || filterToday || filterIfood ? 'var(--red)' : undefined,
              }}>
              <Icon name="filter" size={14} /> Filtros
              {(filterPriority !== 'all' || filterAI || filterToday || filterIfood) && (
                <span style={{ marginLeft: 4, background: 'var(--red)', color: 'white', borderRadius: 9999, fontSize: 10, padding: '1px 6px', fontWeight: 700 }}>
                  {[filterPriority !== 'all', filterAI, filterToday, filterIfood].filter(Boolean).length}
                </span>
              )}
            </button>
            {showFiltDrop && (
    ...
    <>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          {task.fonte === 'analise' && (
            <span style={{ fontSize: 10, fontWeight: 700, padding: '1px 6px', borderRadius: 9999, background: '#FEE2E2', color: '#B91C1C' }}>
              🍔 iFood
            </span>
          )}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--g-600)', display: 'block', marginBottom: 6 }}>
              Resolução <span style={{ fontWeight: 400, color: 'var(--g-400)' }}>(o que foi feito / combinado com o cliente)</span>
            </label>
