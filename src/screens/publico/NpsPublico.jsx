...
const DEFAULT_BRAND_COLOR = '#B70C00';
...
export default function NpsPublico() {
  ...
  useEffect(() => {
    ...
    fetch(`${BRIDGE_URL}/api/publico/nps/${encodeURIComponent(token)}`)
      ...
      .catch(e => {
        setErroMsg(String(e) === 'link_expirado' ? 'Este link expirou.' : String(e));
        setTela(TELA.ERRO);
      });
  }, [token]);
  ...
  // ── Branding e config ─────────────────────────────────────────────────────
  const brand      = dados?.brand || null;
  const brandColor = brand?.theme_color || brand?.color || DEFAULT_BRAND_COLOR;
  const brandName  = brand?.name || 'Consult Delivery';
  const logoUrl    = brand?.logo_url || null;
  const config     = dados?.config || null;
  const nomeLoja   = brandName;

  const tituloNps    = config?.nps_titulo        || `De 0 a 10, qual a probabilidade de você indicar ${nomeLoja} para um amigo ou colega?`;
  const subtituloNps = config?.nps_subtitulo     || null;
  const msgAgradec   = config?.nps_agradecimento || `Sua opinião é muito importante para ${nomeLoja} continuar melhorando.`;
  ...
  if (tela === TELA.LOADING) return (
    <div style={styles.center}>
      <div style={{ ...styles.spinner, borderTopColor: DEFAULT_BRAND_COLOR }} />
    ...
    <div style={styles.page}>
      ...
      <div style={styles.bodyWrap}>
        <div style={styles.card}>
          ...
          <p style={styles.cardSub}>Sua resposta já foi registrada. Obrigado pelo feedback!</p>
        </div>
      </div>
    </div>
  );
      ...
      <div style={styles.page}>
        ...
        <div style={styles.bodyWrap}>
          <div style={styles.card}>
            ...
            <p style={styles.cardSub}>{msgAgradec}</p>
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.bodyWrap}>
        <div style={styles.card}>
          <h1 style={styles.pergunta}>{tituloNps}</h1>
          {subtituloNps && <p style={styles.subtitulo}>{subtituloNps}</p>}
          ...
          <button
            ...
            style={{
              ...styles.btnEnviar,
              background: nota !== null && !enviando ? brandColor : '#D1D5DB',
              cursor:     nota !== null && !enviando ? 'pointer'  : 'not-allowed',
              opacity:    nota !== null && !enviando ? 1          : 0.7,
            }}
            onClick={enviar}
...
const styles = {
  ...
  subtitulo: {
    fontSize: 14, color: '#555', margin: '0 0 16px', textAlign: 'left',
  },
  escalaMobile: { width: '100%', marginBottom: 8 },
  escalaWrap: {
  ...
  escalaLabels: { display: 'flex', justifyContent: 'space-between', padding: '0 4px' },
  escalaLabelLeft:  { fontSize: 11, color: '#9CA3AF' },
  escalaLabelRight: { fontSize: 11, color: '#9CA3AF' },
  notaLabel: { fontSize: 13, fontWeight: 600, margin: '8px 0 16px', minHeight: 18 },
  notaBadge: {
    display: 'inline-block', marginTop: 20, padding: '8px 20px',
    borderRadius: 20, border: '2px solid', fontSize: 22, fontWeight: 800,
  },
  labelComentario: {
  ...
  btnEnviar: {
    ...
    fontSize: 16, fontFamily: "'Montserrat', system-ui, sans-serif",
    transition: 'opacity 0.15s',
  },
  lgpd: { fontSize: 11, color: '#AAA', margin: '14px 0 0', lineHeight: 1.5 },
};
