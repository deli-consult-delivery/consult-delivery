...
function MissionModal({ mission, onClose, onSaved, headers }) {
  ...
  async function handleSave() {
    ...
    try {
      const url    = isEdit ? `${BRIDGE_URL}/api/goals/missions/${mission.id}` : `${BRIDGE_URL}/api/goals/missions`;
...
function ProjectModal({ project, missions, onClose, onSaved, headers }) {
  ...
  async function handleSave() {
    ...
    setSaving(true); setError('');
    try {
      const url    = isEdit ? `${BRIDGE_URL}/api/goals/projects/${project.id}` : `${BRIDGE_URL}/api/goals/projects`;
      const method = isEdit ? 'PATCH' : 'POST';
      const r = await fetch(url, {
        method, headers: { ...headers, 'Content-Type': 'application/json' },
        body: JSON.stringify(form),
...
function GoalModal({ goal, projects, onClose, onSaved, headers }) {
  ...
  async function handleSave() {
    ...
    setSaving(true); setError('');
    try {
      const url    = isEdit ? `${BRIDGE_URL}/api/goals/goals/${goal.id}` : `${BRIDGE_URL}/api/goals/goals`;
      const method = isEdit ? 'PATCH' : 'POST';
      const r = await fetch(url, {
        method, headers: { ...headers, 'Content-Type': 'application/json' },
...
function TaskModal({ task, goals, onClose, onSaved, headers }) {
  ...
  async function handleSave() {
    ...
    setSaving(true); setError('');
    try {
      const url    = isEdit ? `${BRIDGE_URL}/api/goals/tasks/${task.id}` : `${BRIDGE_URL}/api/goals/tasks`;
      const method = isEdit ? 'PATCH' : 'POST';
      const r = await fetch(url, {
        method, headers: { ...headers, 'Content-Type': 'application/json' },
        body: JSON.stringify(form),
...
// ── GoalsScreen principal ─────────────────────────────────────────────────────
export default function GoalsScreen({ tenantDbId, onNavigate }) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const r = await fetch(`${BRIDGE_URL}/api/goals/summary`, { headers });
      ...
      try {
        const [mR, pR, gR] = await Promise.all([
          fetch(`${BRIDGE_URL}/api/goals/missions`,  { headers }),
          fetch(`${BRIDGE_URL}/api/goals/projects`,  { headers }),
          fetch(`${BRIDGE_URL}/api/goals/goals`,     { headers }),
        ]);
        ...
        const goalsWithTasks = await Promise.all(
          (Array.isArray(gData) ? gData : []).map(async g => {
            try {
              const tr = await fetch(`${BRIDGE_URL}/api/goals/tasks?goal_id=${g.id}`, { headers });
  ...
  async function deleteMission(m) {
    ...
    try {
      await fetch(`${BRIDGE_URL}/api/goals/missions/${m.id}`, { method: 'DELETE', headers });
  ...
  async function deleteProject(p) {
    ...
    try {
      await fetch(`${BRIDGE_URL}/api/goals/projects/${p.id}`, { method: 'DELETE', headers });
  ...
  async function deleteGoal(g) {
    ...
    try {
      await fetch(`${BRIDGE_URL}/api/goals/goals/${g.id}`, { method: 'DELETE', headers });
  ...
  async function deleteTask(t) {
    ...
    try {
      await fetch(`${BRIDGE_URL}/api/goals/tasks/${t.id}`, { method: 'DELETE', headers });
  ...
  async function toggleTaskDone(task) {
    ...
    try {
      const r = await fetch(`${BRIDGE_URL}/api/goals/tasks/${task.id}`, {
        method: 'PATCH',
  ...
  return (
    <div style={{ padding: '24px 20px', maxWidth: 960, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        ...
        <button
          onClick={() => setModal({ type: 'mission', item: null })}
