...
CREATE INDEX IF NOT EXISTS agent_memories_agent_tenant_idx ON agent_memories (agent_slug, tenant_id);
CREATE INDEX IF NOT EXISTS agent_memories_expires_at_idx   ON agent_memories (expires_at)
  WHERE expires_at IS NOT NULL;

ALTER TABLE agent_memories ENABLE ROW LEVEL SECURITY;

-- Agentes (service_role) leem/escrevem livremente
CREATE POLICY "service_role_manage_memories"
  ON agent_memories
  FOR ALL
  USING (auth.role() = 'service_role');

-- Membros do tenant podem ler memórias do seu tenant
